...
sort serial year age
rename famsize famsize_old
gen famsize = nchild if (relate <= 201 | relate == 1114 | relate == 1113) & sex == 2
by serial year: egen tempfamsize = min(famsize)
replace famsize = tempfamsize if famsize == .
by serial year: gen headnchild = nchild if relate == 101
by serial year: egen tempheadnchild = min(headnchild)
...
by serial year: egen wifeinlftemp = min(wifeinlf)
replace wifeinlf = wifeinlftemp if wifeinlf == .
gen numkids = headnchild
drop tempheadnchild
foreach n of numlist 1/`kids'{
    by serial year: gen kid`n'age = age[`n'] if relate == 301 & headnchild >= `n' 
    by serial year: egen tempage = min(kid`n'age)
    replace kid`n'age = tempage if kid`n'age == .
    by serial year: gen kid`n'sex = sex[`n'] if relate == 301 & headnchild >= `n' 
    by serial year: egen tempsex = min(kid`n'sex)
    replace kid`n'sex = tempsex if kid`n'sex == .
    by serial year: gen kid`n'elig = kid`n'age >= 0 & kid`n'age < 13
    drop temp*
}
save temp.dta, replace
gen boykid = relate == 301 & sex == 1
by serial year: egen numboys = total(boykid)
gen girlkid = relate == 301 & sex == 2
by serial year: egen numgirls = total(girlkid)
...
by serial year: egen meductemp = min(meduc)
...
by serial year: egen feductemp = min(feduc)
...
by serial year: egen mhsdiptemp = min(mhsdip)
...
by serial year: egen mcdiptemp = min(mcdip)
...
by serial year: egen fcdiptemp = min(fcdip)
replace fcdip = fcdiptemp if fcdip == .

gen parentage = momage
sort serial year relate
duplicates drop serial year, force
